...
class humedad
{
    ...
    function cambiarHumedades($hum){
        $this->ConectarBD();
        $sql= "UPDATE  `invernaderoDB`.`HumeTierra` SET  `minima` =  '".$hum->minima."', `maxima` =  '".$hum->maxima."', `deseada` =  '".$hum->deseada."' WHERE  `HumeTierra`.`idHumedad` ='".$hum->idHumedad."'";
        mysql_query($sql);
    }
